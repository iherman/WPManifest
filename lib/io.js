 ...
 * @param {string} media_type - Expected media media_type. Default is JSON (ie, application/json). Accepted values are HTML, and JSON (including the JSON 'derivatives', ie, application/XXX+json) [T.B.D. the xxx+json variant]
 * @return {Promise} - encapsulating the body of the resource. If the media_type is JSON, this is a JS object; if it is HTML, it is a DOM tree
 */
exports.fetch_resource = async (url, media_type = json_media_type) => {
                ...
                .then((body) => {
                    if(media_type === json_media_type) {
						try {
							...
							reject(`JSON parsing error in ${url}: ${err}`);
						}
                    } else if(media_type === html_media_type) {
                        try {
							resolve(new JSDOM(body));
						} catch(err) {
							reject(`HTML parsing error in ${url}: ${err}`);
						}
                    } else {
                        reject(`Unknown media type: ${media_type}`)
                    }
                })
                .catch((err) => {
