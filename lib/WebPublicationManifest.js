...
class LocalizableString {
    ...
    static initArray(logger, values) {
        const retval = values.map((name) => new LocalizableString(logger, name)).filter((name) => name.__invalid !== true);
...
class Contributor {
     ...
     */
    static initArray(logger, objs) {
        return objs.map((obj) => {
            const type = obj['@type'] || undefined;
            return (type !== undefined && type.includes('Organization')) ? new Organization(logger, obj) : new Person(logger, obj);
...
class Person extends Contributor {
     ...
     */
    static initArray(logger, values) {
        const retval = values.map((person) => new Person(logger, person)).filter((person) => person.__invalid !== true);
        // We also have to check whether these objects are all persons; there are terms for which this is an extra constraint!
        return retval.filter((item) => logger.assert(item._type.includes('Person'),
...
class Organization extends Contributor {
     ...
     */
    static initArray(logger, values) {
        const retval = values.map((name) => new Organization(logger, name)).filter((org) => org.__invalid !== true);
        return retval.filter((item) => logger.assert(item._type.includes('Organization'),
...
class PublicationLink {
    ...
    static initArray(logger, values, base, m_separate) {
        return values.map((name) => new PublicationLink(logger, name, base, m_separate)).filter((link) => link.__invalid !== true);
    }
}
...
class WebPublicationManifest {
    ...
    __a11yterms(term, allowed_terms, a_mode) {
        ...
        const fva = a_mode.map((item) => (check(item) ? item : null)).filter((item) => item !== null);
     ...
     */
    set artist(value) {
        this._artist = Person.initArray(this.__logger, value);
    }
     ...
     */
    set colorist(value) {
        this._colorist = Person.initArray(this.__logger, value);
    }
     ...
     */
    set editor(value) {
        this._editor = Person.initArray(this.__logger, value);
    }
    ...
    set illustrator(value) {
        this._illustrator = Person.initArray(this.__logger, value);
    }
     ...
     */
    set inker(value) {
        this._inker = Person.initArray(this.__logger, value);
    }
     ...
     */
    set letterer(value) {
        this._letterer = Person.initArray(this.__logger, value);
    }
     ...
     */
    set penciler(value) {
        this._penciler = Person.initArray(this.__logger, value);
    }
     ...
     */
    set readby(value) {
        this._readby = Person.initArray(this.__logger, value);
    }
    ...
    set readingOrder(value) {
        this._readingOrder = PublicationLink.initArray(this.__logger, value, this.__base, this.__m_separate);
    }
     ...
     */
    set resources(value) {
        this._resources = PublicationLink.initArray(this.__logger, value, this.__base, this.__m_separate);
    }
     ...
     */
    set links(value) {
        this._links = PublicationLink.initArray(this.__logger, value, this.__base, this.__m_separate);
    }
