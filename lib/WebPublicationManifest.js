    ...
    constructor(logger, obj, base, lang) {
        if (obj instanceof Object) {
            ...
            if (obj['@type']) {
                ...
                logger.warning(this._type.includes('Person') || this._type.includes('Organization'),
...
class Person extends Contributor {
    ...
    static initArray(logger, values, base, lang) {
        ...
        retval = retval.filter((item) => logger.error(item._type.includes('Person'),
...
class Organization extends Contributor {
    ...
    static initArray(logger, values, base, lang) {
        ...
        retval = retval.filter((item) => logger.error(item._type.includes('Organization'),
                                                      `Only an "Organization" object is allowed in this position (${item._name[0].value})`));
        return (retval.length === 0) ? undefined : retval;
    }
}
