...
class LinkedResource {
     ...
     * "encodingFormat", "name", "description", "rel", and "totalDuration".
    ...
    constructor(logger, obj, base, m_separate) {
        ...
        // The fact of using a string interpolation ensures that if the input is a number, it is properly converted into string
        this._totalDuration = `${obj.totalDuration}` || undefined;

        // Check the validity of the totalDuration value; a warning should be issued if it isn't correct
        if (this._totalDuration) {
            logger.assert(npt.parse(this._totalDuration) !== null, `${this._totalDuration} is an invalid NPT value.`, LogLevel.warning);
        }
        ...
        // they should not be lost...
        // The strange idiom to deep clone an object...
        condClone(['type', 'url', 'encodingFormat', 'name', 'description', 'rel', 'totalDuration'], obj, this);
...
class WebPublicationManifest {
    ...
    // ****************************
    // --- DOMString globalDuration
    /**
    * Global duration value of the publication
    * @param {string} globalDuration
    */
    set globalDuration(duration) {
        const dur = `${duration}`;
        if (this.__logger.assert(npt.parse(dur) !== null, `${dur} is an invalid NPT value.`, LogLevel.warning)) {
            this._globalDuration = dur;
        }
    }

    /**
     *
     * <code>DOMString globalDuration;</code>
     *
     * @returns {string}
     */
    get globalDuration() {
        return this._globalDuration;
    }
