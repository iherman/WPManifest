...
class WebPublicationManifest {
    ...
    // ****************************
    /**
     * This is not a "normal" setter, because the value is not "set" by using the value in the manifest
     * but, rather, using the dom to possibly find the right resource. Also, this must by an async method:
     * indeed, and external HTML resource may have to be fetched, which is an inherently async operation.
     *
     * @async
     * @param {JSDOM} dom - the DOM of the primary entry page. Used to look for the TOC unless an explicit resource is provided
     */
    async set_toc(dom) {
        const find_link = (where) => WebPublicationManifest.__find_link(where, 'contents');
        if (this.__toc_searched) {
            // Just to avoid going through this twice...
            return;
        } else {
            // this is the first time...
            this.__toc = undefined;
            this.__toc_searched = true;
        }
        // Locate a toc reference PublicationLink, if any. If it exists, its url member
        // locates the HTML resources for the toc.
        // The toc resource is either the primary entry point (ie, 'dom') or has to be fetched to create a DOM for it
        const toc_reference = find_link(this._readingOrder) || find_link(this._resources);
        let toc_resource;
        ...
        // Try to locate a doc-toc element in the toc resource; if found, that is the one!
        if (this.__logger.assert(toc_resource !== null, 'TOC resource could not be reached or parsed', LogLevel.warning)) {
