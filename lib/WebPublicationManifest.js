...
const { condClone, bcppattern } = require('./utils');
...
class LocalizableString {
    ...
    static initArray(logger, values, lang) {
        ...
        return nameArray.map((name) => new LocalizableString(logger, name, lang));
    }
}
...
class Person extends Contributor {
    static initArray(logger, values, base, lang) {
        ...
        return personArray.map((name) => new Person(logger, name, base, lang));
    }
}
...
class Organization extends Contributor {
    static initArray(logger, values, base, lang) {
        ...
        return orgArray.map((name) => new Organization(logger, name, base, lang));
    }
}
...
class PublicationLink {
    ...
    static initArray(logger, values, base, lang) {
        const nameArray = Array.isArray(values) ? values : [values];
        return nameArray.map((name) => new PublicationLink(logger, name, base, lang));
    }
}
