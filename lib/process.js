...
} = require('./WebPublicationManifest');
const {
    canonicalize
} = require('./CanonicalManifest');
...
async function obtain_manifest(dom) {
    try {
        ...
        if (ref[0] === '#') {
            ...
            manifestObject = process_manifest(manifest_text, dom, script.baseURI, get_attr(script, 'lang'), get_attr(script, 'dir'));
        } else {
 ...
 *  and a "wpm" pointing at a WebPublicationManifest class instance.
 */
function process_manifest(manifest_text, dom, base, lang = '', dir = '') {
    try {
        ...
        const canonical_manifest = canonicalize(logger, manifest_object, dom.window.document, base, lang, dir);
        ...
        const wpm = new WebPublicationManifest(logger, base, dom.window.document.URL !== base);
