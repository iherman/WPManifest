    ...
    try {
        ...
        if (manifest_object.inLanguage) {
            ...
            if (check_lang) {
                ...
                logger.assert(false, `"${manifest_object.inLanguage}" is not a valid language tag`, LogLevel.warning);
                manifest_object.inLanguage = undefined;
            }
        }

        // Populate the final manifest...
        const wpm = new WebPublicationManifest(logger, base, language, dom.window.document.URL !== base);
