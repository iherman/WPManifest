...
function calculate_defaults(dom) {
    const defaults = {
        title      : '',
        title_lang : '',
        url        : ''
    ...
    if (dom !== undefined) {
        ...
        if (title) {
            ...
            if (title_lang) {
                defaults.title_lang = title_lang;
            }
            defaults.title = title.text;
        }
        defaults.url = dom.window.document.URL;
    }
    return defaults;
}
...
class Logger {
     ...
     * @param {string} message - the message that should be logged, possibly
