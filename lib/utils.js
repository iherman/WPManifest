...
* 2. If a logger is not defined (that is the default), an exception is raised. That should be used when the URL is deferenced.
*
...
function check_url(address, logger = undefined) {
    ...
    if (parsed.protocol === null) {
        ...
        if (logger) {
            logger.error(false, `"${address}": Invalid URL: no protocol`);
            return null;
        }
    ...
    if (['http:', 'https:'].includes(parsed.protocol) === false) {
        if (logger) {
            logger.warning(false, `"${address}": URL is not dereferencable`);
            return null;
        }
    ...
    if (retval === undefined) {
        if (logger) {
            logger.warning(false, `"${address}": the URL isn't valid`);
            return null;
        }
    ...
    if (parsed.port !== null && parsed.port <= 1024) {
        if (logger) {
            logger.warning(false, `"${address}": Unsafe port number used in URL (${parsed.port})`);
            return null;
        }
...
class Logger {
    ...
    /**
     * Set a warning if condition is not met.
     *
     * @param {boolean} condition - if false, the message is stored as a warning.
     * @param {string} message - message to be stored in case of unmet condition.
     */
    ...
    /**
     * Set an error if condition is not met.
     *
     * @param {boolean} condition - if false, the message is stored as an error.
     * @param {string} message - message to be stored in case of unmet condition.
     */
